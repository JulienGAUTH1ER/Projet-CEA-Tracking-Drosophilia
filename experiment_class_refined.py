...
import csv
#import pandas as pd
from larva_class_roi_trap_n_rect_refined import Larva
from maze_class_refined import Maze
...
class Experiment():
    ...
    def __init__(self, date_input, genotype, red_light, camera_par, tracking_par, odor, video_path, file_path):
        ...
        self.position = 0
        self.start = 0  # To avoid counting twice
        self.odor_time = 0  # Time at which the choice is made
        self.chamber_time_btw = 0    # Time between chambers
        self.nearby_choices = [[0,0],[0,0]] 
        self.odorTT = [np.nan, np.nan, np.nan, np.nan, np.nan, np.nan]  # To know if the larva successfully reached the odor
        self.direction = [np.nan, np.nan, np.nan, np.nan, np.nan, np.nan]   # To know if the larva went left or right
    ...
    def init_results_files(self,exp_protocol):
        ...
        """
        self.files = ['Results_refined/' + self.file_path + '_raw' + '.csv',
            'Results_refined/' + self.file_path  +'_draw'+ "/",]  
    ...
    def zipfile(self):
        archived = shutil.make_archive(f'Results_refined/{self.genotype}/{self.Exp_ID}','zip', f'{self.Exp_ID}')
